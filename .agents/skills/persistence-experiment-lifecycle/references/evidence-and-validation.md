...
현재 환경에 맞는 wrapper invocation을 사용한다. Validation 재실행은 새로운 execution이다. 이전 실행이 실패했거나 source, 환경 또는 실행 조건이 변경된 뒤 재실행하려면 현재 요청에서 retry가 명시적으로 승인돼야 한다. 기존 validation 승인이나 Evidence를 retry 승인으로 간주하지 않으며, 실패 원인과 변경된 상태를 확인한 뒤에도 승인 없이 자동 retry하지 않는다.
